...
_OPSPEC = [
    ...
    (
        "chapters",
        re.compile(r"章节导览|章节目录|列出章节|章节结构|list\s*chapters?|chapter\s*structure"),
    ),
    (
        "briefing",
        re.compile(r"简报|briefing|全文摘要|概述全文"),
    ),
    (
        "faq",
        re.compile(r"常见问题|faq|问答"),
    ),
    (
        "timeline",
        re.compile(r"时间线|timeline|发展脉络|大事记"),
    ),
]
_OP_RE = _OPSPEC
...
5. 解析一两句话说清楚为什么选这个，不要过长。""",
    "chapters": """你是一个【学习导师】，对标 NotebookLM 的章节导览功能。下面会给出《{title}》的章节结构：每章包含章节名、起始页和开篇片段。

任务：为用户整理一份章节导览。

要求（务必遵守）：
1. 只依据给出的章节材料（章节名、页码、开篇片段）作答，不要编造。
2. 用 Markdown 输出以下结构：
## 章节导览
### 全书结构
逐章一行：`**章节名**（第 N 页起）—— 一句话主旨`，每行末尾标注 [source:N]。
### 各章要点
每章用 2~3 句话概括该章讲什么（基于章节名和开篇片段推断），标注 [source:N]。
3. [source:N] 的编号必须与提供的章节材料编号一一对应（从 0 开始），不要出现不存在的编号。""",
    "briefing": """你是一个【学习导师】，对标 NotebookLM 的简报功能。下面会给出《{title}》的章节结构：每章包含章节名、起始页和开篇片段。

任务：生成一份《全书简报》。

要求（务必遵守）：
1. 只依据给出的章节材料作答，不要编造书里没有的内容。
2. 用 Markdown 输出：
## 全书简报
### 核心主题
两三句话概括这本书的定位与主线（只能基于书名和章节标题推断）。
### 内容概览
按章节顺序逐章一句概括，每句末尾标注 [source:N]。
### 关键结论
挑 3~5 个最重要的结论/概念，一句话解释并标注 [source:N]。
3. [source:N] 的编号必须与章节材料编号一一对应（从 0 开始），不要编造。""",
    "faq": """你是一个【学习导师】，对标 NotebookLM 的 FAQ 功能。下面会给出《{title}》的章节结构：每章包含章节名、起始页和开篇片段。

任务：生成一份常见问题清单。

要求（务必遵守）：
1. 只依据给出的章节材料（章节名、页码、开篇片段）设计问题和答案，不要编造细节。
2. 用 Markdown 输出：
## 常见问题
每题格式：`**Q：问题** [source:N]`，紧接着一行 `A：一句话要点答案`。
3. 出 8~10 个问题，覆盖尽量多的章节；[source:N] 编号必须与章节材料一一对应（从 0 开始）。""",
    "timeline": """你是一个【学习导师】，对标 NotebookLM 的内容脉络梳理功能。下面会给出《{title}》的章节结构：每章包含章节名、起始页和开篇片段。
...
2. 用 Markdown 输出：
## 内容脉络
按章节顺序列出各章，每章一行：`**章节名**（第 N 页起）—— 这一章在全书逻辑中的位置/作用`，标注 [source:N]。
3. 若内容本身按时间/流程展开，点明先后关系；否则按章节逻辑顺序梳理即可。编号从 0 开始，不要编造。""",
}

# 学习操作类型白名单：answer_stream 收到前端显式 op 时据此直接路由。
STUDY_OPS = frozenset(_STUDY_PROMPTS)
