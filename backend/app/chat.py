...
规则：
1. 来自文档的说法必须标注来源 [source:N]（编号与源材料一致）。关键：编号要「句句对应」——每个 [source:N] 紧跟它支撑的那句话，且该编号对应的片段必须真的包含这句话的内容；严禁在段落末尾集中贴一堆编号、或引用一个并不含该论断的片段。宁可少引用，也不要张冠李戴。
2. 要有教学的判断：
   - 材料充分 → 直接基于材料作答并带引用；
...
3. 内容必须从检索片段里抠「真东西」：点名书中特有的名词、定义、寄存器/引脚/参数/数值、流程步骤，能对上原文位置就加 [source:N]（编号与源材料一致，不要编造编号）。段落尽量做到「句句有出处」——[source:N] 紧跟它支撑的那句话，且该编号片段必须真的包含该内容；没有片段支撑的推论/补充，用「补充：我自己的讲解」明确标出，不得冒充书里内容。
